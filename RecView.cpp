...
{
	const int pageHeight = pDC->GetDeviceCaps(VERTRES);
	const int pageWidth = pDC->GetDeviceCaps(HORZRES);
	int textHeight = pDC->GetTextExtent("A").cy;

	int borderY = pageHeight / 30;
	int borderX = pageWidth / 15;
	int column1 = borderX;
	int column2 = borderX * 5;
	int column3 = borderX * 10;

	int y = borderY;

	pDC->TextOut(column1, y, "id");
	pDC->TextOut(column2, y, "name");
	pDC->TextOut(column3, y, "manager");
	...
	while (!rs.IsEOF())
	{
		...
		pDC->TextOut(column1, y, id);
		pDC->TextOut(column2, y, rs.m_name);
		pDC->TextOut(column3, y, rs.m_manager ? _T("X") : _T(""));
